...
function hostNextRound(data) {
  if(data.round < wordPool.length ){
    ...
    sendExpr(data.round, data.gameId);
  } else {
...
function playerAnswer(data) {
  ...
  console.log('data in playerAnswer: ', data);
...
function playerWrongAnswer(data) {
  io.sockets.in(data.gameId).emit('markAnswerAsWrong', data);
}
 ...
 */
function sendExpr(wordPoolIndex, gameId) {
  //var data = getWordData(wordPoolIndex);
  var data = generateExpression(wordPoolIndex);
...
function generateExpression(round) {
  var answer;
  var operator;
  var expression;
  var sum = 100;
  ...
  // Generate two numbers in range (11, 89) to be used in the expression.
  while (sum > 99 || diff < 1) {
    var firstNum = Math.floor(Math.random() * 78 + 11);
    var secondNum = Math.floor(Math.random() * 78 + 11);
  ...
  options.push(result);
  for (var i = 0; i < 5; i++) {
    options.push(Math.floor(Math.random() * 78 + 11));
  }
...
function generateExpressions(gameId) {
  for (var i = 0; i < 10; i++) {
  expressions.push(generateExpression(gameId));
  }
}

generateExpression(98);
